<?php
    ...
    class CareerController {
        ...
        public function ShowPersonalCareer() {
            $careerListApi = $this->careerDAO->GetAll();
            $studentListapi = $this->studentDAO->GetAllApi();
            $loggedStudent = NULL;
            foreach ($careerListApi as $keyApi=>$valueApi) {
                foreach($studentListapi as $keypdo=>$valuepdo) {
                    if($valueApi->getCareerId() == $valuepdo->getCareerId()) {
                        $loggedStudent = $valueApi;
                    }
                }
            }
